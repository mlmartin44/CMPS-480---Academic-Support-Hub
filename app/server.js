...
async function resolveAuthorId({ authorId, author }) {
  ...
  if (author && author.trim()) {
    ...
    const r3 = await query(
      `SELECT UserID FROM User WHERE FirstName = :first LIMIT 1`,
      { first: author.trim() }
    );
    if (r3.length) return r3[0].UserID;
  }
  ...
  try {
    ...
    const rows = await query(
      `SELECT
         ...
         COALESCE(CONCAT(u.FirstName, ' ', u.LastName), u.Email, CONCAT('User#', u.UserID)) AS author
       FROM Question q
...
// POST accepts { courseId?, course?, author?, authorId?, content } and inserts
